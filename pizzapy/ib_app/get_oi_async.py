...
async def get_expiries_and_strikes_async(symbol: str, ib: IB) -> tuple[list[str] | None, list[float] | None]:
    ...
    USED BY: get_all_option_contracts
    """

    handcrafted_stock: Stock = Stock(symbol, "SMART", "USD")

    [qualified_stock] = await ib.qualifyContractsAsync(handcrafted_stock)
...
async def get_single_qualified_option_async(option: Option, ib: IB) -> Optional[Option]: 

    try:
        
        # If IB can’t find a matching contract, this will be [None]
        [qualified_contract] = await ib.qualifyContractsAsync(option)

        if qualified_contract is None:
            raise
        else:
            print(f'{qualified_contract}')
            return qualified_contract

    except:
        print(f'get_single_qualified_option_async', e)
    # END OF get_single_qualified_option_async()




async def runner(coro, semaphore):
    """
...
async def get_contract_oi_async(option: Option, ib: IB) -> OptionPosition | None:
    ...
    def on_tick(ticker):
        if option.right == "C":
            condition_met = str(ticker.close) != 'nan' and str(ticker.callOpenInterest) != 'nan'
        else:
            condition_met = str(ticker.close) != 'nan' and str(ticker.putOpenInterest) != 'nan'
...
async def gather_positions_async(call_options: list[Option], put_options: list[Option]) -> Any:
    ...
    while working_calls:
        ...
        call_wrapped = [ runner(coro, sem) for coro in call_option_tasks ]
    ...
    while working_puts:
        ...
        put_wrapped = [ runner(coro, sem) for coro in put_option_tasks ]
